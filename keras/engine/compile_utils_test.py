...
class LossesContainerTest(test_combinations.TestCase):
    ...
    @test_combinations.generate(
        test_combinations.combine(
            input_type=["masked", "ragged"],
            reduction=['auto', 'sum'],
            sw=[True, False]),)
    def test_all_masked_loss(self, input_type, reduction, sw):
        y_p = tf.ragged.constant([[[1], [1], [1]], 
                                  [[1], [1]]], dtype=tf.float32)
        y_t = tf.ragged.constant([[[1], [0], [0]],
                                  [[1], [1]]], dtype=tf.float32)
